...
using GroupDocsComparisonVisualStudioPlugin.GUI;
using GroupDocsComparisonVisualStudioPlugin.Properties;

namespace GroupDocsComparisonVisualStudioPlugin
{
    public class Connect : Extensibility.IDTExtensibility2, IDTCommandTarget
    {
        ...
        private const string GROUPDOCS_MENU_TOOLTIP = "Download and Run GroupDocs.Comparison for .NET API Examples";

        private const string Comparison_COMMAND_NAME = "GroupDocsComparisonVSPlugin";
        private const string Comparison_COMMAND_CAPTION = "New GroupDocs.Comparison Example Project";
        private const string Comparison_COMMAND_TOOLTIP = "Download and Run GroupDocs.Comparison for .NET API Examples";
            ...
            try
            {
                ...
                for (int iloop = 1; iloop <= toolsPopup.CommandBar.Controls.Count; iloop++)
                {
                    if (toolsPopup.CommandBar.Controls[iloop].Caption == GROUPDOCS_MENU_CAPTION)
                    {
                        ...
                        {
                            if (cmdbtn.Caption == Comparison_COMMAND_CAPTION)
                            {
                                oBtn = cmdbtn;
                            }
                        }
                ...
                if (oBtn == null)
                {
                    ...
                    Command myCommand = commands.AddNamedCommand2(addInInstance, Comparison_COMMAND_NAME,
                        Comparison_COMMAND_CAPTION, Comparison_COMMAND_TOOLTIP, false, Resources.pnglogosmall, ref contextGUIDS,
           ...
           ref object varOut, ref bool handled)
        {
            try
            {
                ...
                {
                    if (cmdName == addInInstance.ProgID + "." + Comparison_COMMAND_NAME)
                    {
